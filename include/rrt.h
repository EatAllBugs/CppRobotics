...
RRT::RRT(Node *start_, Node *end_, std::vector<std::vector<float> > ob_list_,
  std::vector<float> rand_area_, float expand_dis_ = 1.0,
  float path_resolution_ = 1.0, int goal_sample_rate_ = 5, int max_iter_ = 500):
  start(start_), end(end_), ob_list(ob_list_), expand_dis(expand_dis_),
  path_resolution(path_resolution_), goal_sample_rate(goal_sample_rate_),
  max_iter(max_iter_), goal_gen(goal_rd()),
  goal_dis(std::uniform_int_distribution<int>(0, 100)), area_gen(area_rd()),
  area_dis(std::uniform_real_distribution<float>(rand_area_[0], rand_area_[1])),
...
std::vector<Node *> RRT::planning() {
  ...
  cv::circle(bg, cv::Point((int)((start->x - rand_area[0])*img_reso),
      (int)((start->y - rand_area[0])*img_reso)), 20, cv::Scalar(0, 0, 255), -1);
  cv::circle(bg, cv::Point((int)((end->x - rand_area[0])*img_reso),
      (int)((end->y - rand_area[0])*img_reso)), 20, cv::Scalar(255, 0, 0), -1);
  for (auto item : ob_list) {
    cv::circle(bg, cv::Point((int)((item[0] - rand_area[0])*img_reso),
        (int)((item[1] - rand_area[0])*img_reso)), (int)item[2] * img_reso,
      cv::Scalar(0, 0, 0), -1);
  }
  ...
  while (true) {
    ...
    cv::line(
      bg,
      cv::Point((int)((new_node->x - rand_area[0])*img_reso),
        (int)((new_node->y - rand_area[0])*img_reso)),
      cv::Point((int)((nearest_node->x - rand_area[0])*img_reso),
        (int)((nearest_node->y - rand_area[0])*img_reso)),
    ...
    cv::imshow("rrt", bg);
    cv::waitKey(5);
    if (std::sqrt(std::pow((new_node->x - end->x),
          2) + std::pow((new_node->y - end->y), 2)) <= expand_dis) {
  ...
  while (temp->parent != NULL) {
    ...
    cv::line(
      bg,
      cv::Point((int)((temp->x - rand_area[0])*img_reso),
        (int)((temp->y - rand_area[0])*img_reso)),
      cv::Point((int)((temp->parent->x - rand_area[0])*img_reso),
        (int)((temp->parent->y - rand_area[0])*img_reso)),
...
Node *RRT::GetNearestNode(const std::vector<float> rnd) {
  ...
  for (int i = 0; i < node_list.size(); i++) {
    float dist = std::pow((node_list[i]->x - rnd[0]),
        2) + std::pow((node_list[i]->y - rnd[1]), 2);
...
bool RRT::CollisionCheck(Node *node) {
  for (std::vector<float> item : ob_list) {
    if (std::sqrt(std::pow((item[0] - node->x), 2) + std::pow((item[1] - node->y),
          2)) <= item[2]) {
      return false;
    }
  }
...
Node *RRT::steer(Node *from_node, Node *to_node,
  float extend_length = std::numeric_limits<float>::max()) {
...
std::vector<float> RRT::calc_distance_and_angle(Node *from_node,
  Node *to_node) {
