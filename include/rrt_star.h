...
class RRTStar: public RRT {
 public:
  RRTStar(Node *, Node *, std::vector<std::vector<float> >, std::vector<float>,
...
RRTStar::RRTStar(Node *start_, Node *end_,
  std::vector<std::vector<float> > ob_list_, std::vector<float> rand_area_,
  float expand_dis_, float path_resolution_, int goal_sample_rate_, int max_iter_,
  float connect_circle_dist_): RRT(start_, end_, ob_list_, rand_area_,
      expand_dis_, path_resolution_, goal_sample_rate_, max_iter_),
...
std::vector<Node * > RRTStar::find_near_nodes(Node *new_node) {
  ...
  for (Node *n_ : node_list) {
    if ((n_->x - new_node->x) * (n_->x - new_node->x) + (n_->y - new_node->y) *
      (n_->y - new_node->y) < r * r) {
      output.push_back(n_);
    }
    return output;
