...
Matrix5f solve_DARE(Matrix5f A, Matrix52f B, Matrix5f Q, Eigen::Matrix2f R) {
  ...
  for (int i = 0; i < maxiter; i++) {
    Matrix5f Xn = A.transpose() * X * A - A.transpose() * X * B *
...
Vec_f lqr_steering_control(State state, Vec_f cx, Vec_f cy, Vec_f cyaw,
  Vec_f ck, Vec_f sp, float &pe, float &pth_e) {
  int ind = 0;
...
void closed_loop_prediction(Vec_f cx, Vec_f cy, Vec_f cyaw, Vec_f ck,
  ...
  Vec_f y_h;
  while (T >= time_) {
    Vec_f control = lqr_steering_control(state, cx, cy, cyaw, ck, speed_profile, e,
        e_th);
