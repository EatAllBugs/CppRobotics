 ...
 public:
  ...
  void operator()(ADvector &fg, const ADvector &vars) {
    ...
    for (int i = 0; i < T - 1; i++) {
      ...
      fg[0] += 0.5 * CppAD::pow(traj_ref(2,
            i + 1) - (yaw0 + v0 * CppAD::tan(delta0) / WB * DT), 2);
...
void mpc_simulation(Vec_f cx, Vec_f cy, Vec_f cyaw, Vec_f ck,
  Vec_f speed_profile, Poi_f goal) {
  State state(cx[0], cy[0], cyaw[0], speed_profile[0]);
  if ((state.yaw - cyaw[0]) >= M_PI) {
    state.yaw -= M_PI * 2.0;
  } else if ((state.yaw - cyaw[0]) <= -1.0 * M_PI) {
    state.yaw += M_PI * 2.0;
  }
  float goal_dis = 0.5;
  int iter_count = 0;
  ...
  while (MAX_TIME >= iter_count) {
    calc_ref_trajectory(state, cx, cy, cyaw, ck, speed_profile, 1.0, target_ind,
      xref);
    ...
    cv::line(
      ...
      cv_offset(state.x + std::cos(state.yaw)*WB * 2,
        state.y + std::sin(state.yaw)*WB * 2, bg.cols, bg.rows),
      cv::Scalar(255, 0, 255),
      15);
    cv::line(
      bg,
    ...
    cv::line(
      bg,
      cv_offset(state.x + std::cos(state.yaw)*WB * 2 + std::cos(
          state.yaw + steer) * 0.5,
