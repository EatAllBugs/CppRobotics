...
StateList calc_uniform_polar_states(int nxy, int nh, int d,
  ...
  StateList states = sample_states(angle_samples, a_min, a_max, d, p_max, p_min,
      nh);
  return states;
};
...
StateList calc_lane_states(float l_center, float l_heading, float l_width,
  float v_width, float d, int nxy) {
...
Parameter search_nearest_one_from_lookuptable(TrajState target,
  Table csv_file) {
