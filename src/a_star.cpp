...
class Node {
 public:
  ...
  Node(int x_, int y_, float sum_cost_ = 0, Node *p_node_ = NULL): x(x_), y(y_),
...
void a_star_planning(float sx, float sy,
  ...
  std::vector<std::vector<float> > path_cost(xwidth, vector<float>(ywidth,
  ...
  while (true) {
    ...
    for (int i = 0; i < motion.size(); i++) {
      ...
      if (path_cost[node->x][node->y] + motion[i].sum_cost <
        ...
        path_cost[new_node->x][new_node->y] = path_cost[node->x][node->y] +
          motion[i].sum_cost;
        pq.push(new_node);
      }
    }
